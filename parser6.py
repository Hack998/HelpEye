...
from ast6 import Number, Inc, Dec, Inclination, Call, Procedure, Variable, Brightness, Dow, Vibration, Move, For, Temperature, Object, Sounds, Case, When
    ...
    def __init__(self, module, builder, printf):
        self.pg = ParserGenerator(
            ...
            ['DECLARE', 'EQUAL', 'NUMBER', 'INC', 'DEC', 'OPEN_PAREN', 'CLOSE_PAREN',
             ...
             'IMPORT', 'CALL', 'PROCEDURE', 'BEGIN', 'END', 'MAIN', 'BRIGHT',
    ...
    def parse(self):
        ...
        def declare(p):
            if self.token == 0:
                ...
                raise SystemExit("No se pueden hacer declaraciones en el Main")
                return
            elif self.token == 6:
        ...
        def reDeclare(p):
            if self.token == 0:
                ...
                self.declarations = Variable(p[0].value,p[2].value).eval(self.declarations)
            elif self.token == 6:
        ...
        def IncP(p):
            if self.token == 0:
                ...
                right = Number(p[4].value)
                self.name = self.name + "r" 
                y = (Inc(self.builder, self.module,self.printf,left, right, self.name)).evalc(self.declarations,[])
                self.declarations = y[0]
                return
            elif self.token == 6:
                left = p[2].value
                right = Number(p[4].value)
        ...
        def DecP(p):
            if self.token == 0:
                ...
                right = Number(p[4].value)
                self.name = self.name + "r" 
                y = (Dec(self.builder, self.module,self.printf,left, right, self.name)).eval(self.declarations)
                self.declarations = y[0]
                return
            elif self.token == 6:
                left = p[2].value
        ...
        def InclinationP(p):
            if self.token == 0:
                ...
                self.name = self.name + "r" 
                return (Inclination(self.builder, self.module,self.printf,p[2].value,self.name)).evalc(self.declarations,[])
            elif self.token == 6:
                self.name = self.name + "r" 
        ...
        def ObjectP(p):
            if self.token == 0:
                ...
                self.name = self.name + "r" 
                return (Object(self.builder, self.module,self.printf,p[2].value,self.name)).evalc(self.declarations,[])
            elif self.token == 6:
                self.name = self.name + "r" 
        ...
        def SoundsP(p):
            if self.token == 0:
                ...
                self.name = self.name + "r" 
                return (Sounds(self.builder, self.module,self.printf,p[2].value,self.name)).evalc(self.declarations,[])
            elif self.token == 6:
                self.name = self.name + "r" 
        ...
        def BrightnessP(p):
            if self.token == 0:
                ...
                self.name = self.name + "r" 
                return (Brightness(self.builder, self.module,self.printf,p[2].value,self.name)).evalc(self.declarations,[])   
            elif self.token == 6:
                self.name = self.name + "r" 
        ...
        def TemperatureP(p):
            if self.token == 0:
                ...
                self.name = self.name + "r" 
                return (Temperature(self.builder, self.module,self.printf,p[2].value,self.name)).evalc(self.declarations,[])   
            elif self.token == 6:
                self.name = self.name + "r" 
        ...
        def VibrationP(p):
            if self.token == 0:
                ...
                self.name = self.name + "r" 
                return (Vibration(self.builder, self.module,self.printf,p[2].value,self.name)).evalc(self.declarations,[])
            elif self.token == 6:
                self.name = self.name + "r" 
        ...
        def MoveP(p):
            if self.token == 0:
                ...
                self.name = self.name + "r" 
                return (Move(self.builder, self.module,self.printf,p[2].value,self.name)).eval(self.declarations,[])
            elif self.token == 6:
                self.name = self.name + "r" 
        ...
        def ImportP(p):
            if self.token == 0:
                ...
                pg = Parserr(self.module, self.builder, self.printf, self.name)
                pg.parse()
                parser = pg.get_parser()
                g = parser.parse(tokens)
                
                for p in g[0]:
                    self.declarations.append(p)
        ...
        def CallNP(p):
            if self.token == 0:
                ...
                u = (Call(p[1].value)).eval(self.procedures, self.declarations)
                self.declarations = u[0]
                self.procedures = u[1]
                return
            elif self.token == 6:
        ...
        def CallEP(p):
            if self.token == 0:
                ...
                u = (Call(p[1].value, self.arguments)).eval(self.procedures, self.declarations)
                self.declarations = u[0]
                self.procedures = u[1]
                self.arguments = []
                return
